...
function elfBattle() {
    ...
    battleText(elf.name + "가 " + orc.name + "를 공격,\n " + orc.name + "는 [" + elfDmg + "]의 대미지를 받았습니다. \n");
    battleText(orc.name + "의 현제 체력은 " + orc.hp + "입니다. \n");
}
function orcBattle() {
    var orcDmg = ranAtk(orc.attack);
    elf.hp = elf.hp - orcDmg;
    battleText(orc.name + "가" + elf.name + "를 공격,\n" + elf.name + "는[" + orcDmg + "]의 대미지를 받았습니다.\n\n");
    ...
    while (true) {
        ...
        if (elf.hp <= 0 || orc.hp <= 0) {
            if (elf.hp <= 0) {
                elf.exp = 0;
                hr()
                battleText("\n" + elf.name + "의 HP가" + elf.hp + "이되어 전투에서 패배하였습니다.\n");
